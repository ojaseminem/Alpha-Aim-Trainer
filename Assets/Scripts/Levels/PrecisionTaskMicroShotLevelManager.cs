...
namespace Levels
{
    public class PrecisionTaskMicroShotLevelManager : MonoBehaviour
    {
        #region Singleton

        public static PrecisionTaskMicroShotLevelManager Instance;
        ...
        private void Start()
        {
            ChangeState(PrecisionTaskMicroShot.PreGame);
            maxTargetCount = 30;
        }

        private void ChangeState(PrecisionTaskMicroShot flickingTaskGridShot)
        {
            switch (flickingTaskGridShot)
            {
                case PrecisionTaskMicroShot.PreGame:
                    SetVariables();
                    UnlockAimDisableCursor();
                    StartCoroutine(StartTask());
                    break;
                case PrecisionTaskMicroShot.Game:
                    SpawnTargets();
                    break;
                case PrecisionTaskMicroShot.PostGame:
        ...
        private IEnumerator StartTask()
        {
            ...
            ChangeState(PrecisionTaskMicroShot.Game);
        }
        ...
        public void IncrementHits()
        {
            ...
            _currentTargetCount++;
            if(_currentTargetCount >= maxTargetCount) ChangeState(PrecisionTaskMicroShot.PostGame);
        }
    ...
    public enum PrecisionTaskMicroShot
    {
        PreGame,
        Game,
        PostGame
